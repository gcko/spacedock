...
- **Cycle 4 → Cycle 5 (2026-05-01).** Captain rejected cycle-4's ideation gate. Path B + Path C architectural choice stays; this was refinement, not redesign. Staff review surfaced 5 concrete fixes, all addressed in cycle 5: (1) expanded the DONE-claim short-list to include `merged`, `opened PR`, `archived`, with MCP-tool variants and a session-window scope rule preventing pre-session-window false-positives, surfaced via an explicit evidence-class abstraction; (2) replaced AC-F1's `route back` anchor with the distinctive `unmatched DONE claim` to avoid false-positive grep matches against the unrelated Feedback Rejection Flow at shared-core lines 124, 165, 167; (3) AC-F4 now requires BOTH intakes (Claude Code CLI repo + Anthropic model/API support), not a single combined post; (4) helper invocation `claude-team verify-completion` is now REQUIRED in Implementation Scope and AC-F1 (was leave-it-open in cycle 4); (5) AC-F2 expanded from 2 cases (positive + negative) to 3 cases (positive + negative + prior-session-evidence) — the prior-session case asserts the session-window scope rule from fix 1. Operational concerns from the staff review (MCP tool name mapping, routing order between tool-call and AC cross-checks, inheriting the once-only loop bound from existing step 3) are surfaced in Implementation Scope item 1's specification list (a)-(e).
...
**Advance #177 to implementation against Path B (FO-side post-completion verification, implemented as a `claude-team verify-completion` helper invoked from `## Completion and Gates`) as the primary mitigation, with Path C (file upstream issues at BOTH the Claude Code CLI intake AND the Anthropic model/API support intake) as a parallel low-cost lift. Path A (section compression) is rejected as incomplete.**
...
- **Path A (section compression).** Cycle-3 AC-R2 PASS proves the per-teammate prose body is load-bearing on the priming surface for the standing-teammate fixture. **But the dispatch-prompt-shape priming surface is not just the standing-teammates section.** Tonight's PR #181 opus-tier failures land on `test_dispatch_completion_signal` (uses fixture `completion-signal-pipeline` — verified at `tests/fixtures/completion-signal-pipeline/` to have NO `_mods/` directory and NO standing teammates) AND `test_feedback_keepalive` (uses fixture `keepalive-pipeline` — independently verified at `tests/fixtures/keepalive-pipeline/` to have NO `_mods/` directory and NO standing teammates). Both failing fixtures are team-mode dispatches that DO NOT include the standing-teammates section at all. Compressing only the standing-teammates section therefore CANNOT fix those tests. To cover them under Path A would require also compressing the Completion Signal block, the team-mode framing, the checklist boilerplate, and any other prompt block that shows up in the failing fixtures — a priming-whack-a-mole game with no upper bound. This rules Path A out as a complete mitigation. Cycle-3's empirical PASS on the standing-teammate fixture remains as opportunistic evidence, not a strategy.
- **Path B (post-completion verification).** A cross-cutting fix at the FO contract layer (`first-officer-shared-core.md` `## Completion and Gates`, between "review checklist items" and "AC coverage cross-check"). When a worker completes, the FO already reads the latest `## Stage Report` and reviews items against the dispatched checklist. Path B inserts a step that scans the worker's own session-stamped `fo-log.jsonl` (session-window scope only — prior worker sessions on the same worktree are not in obligation scope) for evidence of the claimed DONE actions, mapped through an evidence-class abstraction (each DONE-claim phrase like `committed: ...` accepts one or more tool_use shapes — `Bash` + `git commit` substring, `mcp__git__commit`, `mcp__*__commit_*` MCP variants — under a single named class). When a stage report claims DONE for an evidence class that has no matching tool_use in the current session window, the FO routes the worker back to repair the report (existing convention in step 3, inheriting its once-only loop bound) rather than accepting it. The cross-check is implemented as a `claude-team verify-completion` helper invocation, not inlined matcher logic in shared-core prose — this gives unit-test reach for AC-F2 and provides a natural FO-on-opus self-defense against silent prose-only matchers being hallucinated past. This addresses the regression's *symptom* (DONE claims without tool calls) regardless of which prompt surface caused the priming, and extends the streaming-watcher discipline (#173/#175) — currently confined to test assertions — into production. The infrastructure (`scripts/test_lib.py:1213 tool_use_matches`, `scripts/test_lib.py:1288 FOStreamWatcher`) is already present; Path B reuses the same fo-log inspection shape.
...
1. **Path B implementation in shared-core.** Edit `skills/first-officer/references/first-officer-shared-core.md` `## Completion and Gates` to add a stage-report-vs-tool-call cross-check step between the existing "review checklist items" step and "AC coverage cross-check." The new step must specify (a) which fo-log path is read (worker's session-stamped jsonl), (b) the **session-window scope rule** — the matcher reads ONLY the worker's own session-stamped fo-log entries; tool-call evidence emitted in a *prior* worker session (e.g., a commit made by an earlier ensign on the same worktree, then referenced in a later worker's stage report) is OUT of obligation scope and does NOT need to match in the current session window, (c) the rule for matching DONE claims to tool-call evidence via an **evidence-class abstraction** (see item 2), (d) the failure action — route back via existing step 3 of `## Completion and Gates`, inheriting the existing **once-only loop bound** (the FO sends a worker back at most once for repair; if a re-dispatched worker still fails the cross-check, escalate per existing escalation conventions, do NOT introduce an unbounded retry loop), (e) the **routing order** when both this cross-check AND the AC coverage cross-check fail in the same FO turn — the tool-call cross-check fires first because it is cheaper to repair (concrete missing tool call vs. abstract missing AC evidence) and a worker that fails the tool-call cross-check is unlikely to satisfy the AC cross-check until the underlying fabrication is repaired.
2. **DONE-claim short-list and evidence-class abstraction (load-bearing).** The cross-check operates on an explicit short-list of DONE-claim phrases that obligate a tool-call match. Minimum short-list (each maps to an "evidence class" that accepts ONE OR MORE tool name + input shapes — implementation should name the abstraction in code so that adding new tool variants is a one-line edit):
   - `committed: ...` → evidence class `commit_made` accepts `Bash` tool_use with `git commit` substring OR `mcp__git__commit` OR any `mcp__*__commit_*` MCP variant.
   - `pushed: ...` / `pushed to ...` → evidence class `push_made` accepts `Bash` tool_use with `git push` substring OR `mcp__git__push` OR `mcp__*__push_*` MCP variant.
   - `merged: ...` / `merged PR ...` → evidence class `merge_made` accepts `Bash` tool_use with `gh pr merge` substring OR `mcp__github__merge_pull_request` OR `mcp__*__merge_*` MCP variant.
   - `opened PR ...` / `created PR ...` → evidence class `pr_opened` accepts `Bash` with `gh pr create` substring OR `mcp__github__create_pull_request` OR `mcp__*__create_pr*`.
   - `archived: ...` / `archived to ...` → evidence class `archive_made` accepts `Bash` tool_use with `mv` or `git mv` to an `_archive` path substring OR `mcp__filesystem__move` (or equivalent) targeting an `_archive` path.
   - `sent SendMessage to {agent}` / `sent message to {agent}` → evidence class `sendmessage_made` accepts `SendMessage` tool_use with `to: {agent}` input substring.
   - `wrote {file}` / `created {file}` → evidence class `write_made` accepts `Write` tool_use OR `Edit` tool_use OR `mcp__filesystem__write_file` targeting the named path.
   Claims OUT of obligation scope (do NOT need to match — they describe reads/cognition, not external mutations): `read ...`, `surveyed ...`, `reviewed ...`, `confirmed ...`, `noted ...`, `decided ...`. The evidence-class abstraction is named in shared-core prose as the load-bearing concept; the implementation worker MUST NOT inline a flat tool-name list because adding a new MCP variant would then require editing prose in three places instead of one.
3. **Helper invocation (REQUIRED, not optional).** Implement the cross-check as `claude-team verify-completion --name {worker} --fo-log {path}` returning structured pass/fail (exit code + JSON or formatted stdout naming the unmatched DONE claim and its expected evidence class). Do NOT inline the matcher logic in shared-core prose — the prose names the contract (session window scope, evidence-class abstraction, routing order, once-only loop bound) and points at the helper for the matcher implementation. Rationale: (a) testability — AC-F2 runs the helper directly against synthesised fo-logs without needing a live FO session; (b) FO-on-opus self-defense — an inline matcher in markdown prose could itself be hallucinated past, but a tool invocation cannot be silently faked; (c) consistency with the established `claude-team build` / `claude-team context-budget` convention at `first-officer-shared-core.md:116, 141`. Reuse `scripts/test_lib.py:1213 tool_use_matches`'s shape; do NOT duplicate the matcher logic.
4. **Test coverage.** Unit tests against synthesised fo-logs (covered by AC-F2). The streaming-watcher pattern (`w.expect(tool_use_matches(...))`) at `scripts/test_lib.py:1213` is the test-side convention; the new test asserts the FO-side equivalent (the helper invocation) fires in production.
5. **Path C — file upstream issue (BOTH intakes).** Two parallel intakes — one per surface, since this is a regression with evidence on BOTH the CLI runtime AND the model:
   - **CLI intake**: a short issue at `https://github.com/anthropics/claude-code/issues` describing the FO-on-`claude-opus-4-7` regression visible across Claude Code versions 2.1.111-2.1.121 (low/medium effort hallucinates DONE in stage reports without emitting tool calls).
   - **Model intake**: a parallel report through Anthropic's model/API support channel (the public-issue equivalent if one exists, or the documented support form / email intake) describing the model-behavior regression on `claude-opus-4-7` at low/medium effort, citing the same fo-log artifacts.
   Both intakes cite the cycle-2 + cycle-3 fo-log artifacts as starting reproducers (already in `/tmp/cycle3/` and from cycle-2's `/tmp` set). Path C does NOT block Path B's merge; both intakes ship in parallel with Path B implementation.

### Acceptance Criteria

**AC-F1 — Shared-core specifies the stage-report-vs-tool-call cross-check (with helper).**

- Verify: `grep -nE "tool-call evidence|unmatched DONE claim" skills/first-officer/references/first-officer-shared-core.md` shows a new block in `## Completion and Gates` describing the cross-check rule. The block names: (a) which fo-log path is read (worker's session-stamped jsonl), (b) the **session-window scope rule** (tool-call evidence from prior worker sessions does NOT obligate a match in the current session), (c) the **evidence-class abstraction** (each DONE-claim phrase maps to an evidence class accepting one or more tool_use shapes; named in prose, implemented in the helper), (d) the failure action — route through existing `## Completion and Gates` step 3 with its inherited once-only loop bound, (e) the routing order — tool-call cross-check fires before AC coverage cross-check, (f) the helper invocation `claude-team verify-completion --name {worker} --fo-log {path}` is the implementation surface; shared-core prose names the contract, the helper holds the matcher logic.
- Pass: a future FO reading this section can implement the cross-check by invoking the helper, with the contract (session window, evidence classes, routing order, once-only loop bound) determined from the prose alone.
- Verified by: a static test in `tests/test_first_officer_shared_core.py` (or new sibling) asserting the cross-check block exists in `## Completion and Gates` and contains the anchor phrases **`tool-call evidence`** and **`unmatched DONE claim`** (the latter replaces the cycle-4 anchor `route back`, which appears in three other places in shared-core under the unrelated Feedback Rejection Flow at lines 124, 165, 167 and would create false-positive grep matches). Both anchors must occur within the `## Completion and Gates` section, not elsewhere.

**AC-F2 — Cross-check fires in unit tests across three cases (positive, negative, prior-session-evidence).**

- Verify: three cases exercise `claude-team verify-completion`:
  - **POSITIVE**: stage report claims `committed: refactored helper`; fo-log for the current worker session contains a `Bash` tool_use with `git commit` substring. Helper exits 0 (cross-check passes).
  - **NEGATIVE**: stage report claims `committed: refactored helper`; fo-log for the current worker session contains NO matching tool_use. Helper exits non-zero with stdout/JSON naming the unmatched DONE claim and the expected evidence class (`commit_made`).
  - **PRIOR-SESSION**: stage report claims `committed: refactored helper`; current worker session's fo-log contains NO matching tool_use; a *prior* session's fo-log on the same worktree DOES contain the matching `Bash` `git commit` tool_use. Helper exits 0 (cross-check passes — session-window scope rule allows prior-session evidence to satisfy the obligation, OR more precisely, the obligation does not apply to claims about pre-session-window actions). The exact behavior the session-window scope rule from AC-F1 (b) demands.
- Pass: all three cases assert as documented; the negative case names the specific unmatched claim and evidence class in helper output.
- Verified by: a new test file under `tests/` (likely `tests/test_verify_completion.py`) with at least the three named cases — positive, negative, prior-session-evidence — each calling the helper directly with a synthesised fo-log fixture and asserting on exit code + (for negative) the named unmatched claim.
...
**AC-F4 — Path C upstream issues filed at BOTH intakes with attached artifacts.**

- Verify: TWO upstream reports exist, one per surface (CLI runtime + model behavior):
  - **(a) CLI intake**: a public issue at `https://github.com/anthropics/claude-code/issues/...` describing the FO-on-`claude-opus-4-7` regression visible across Claude Code versions 2.1.111-2.1.121, with the cycle-3 fo-logs (`/tmp/cycle3/ac-r{1,2,3}-fo-log.jsonl`) attached or linked to a gist.
  - **(b) Model intake**: a parallel report through Anthropic's model/API support channel (a public issue at the appropriate Anthropic API intake if one exists, or the documented support form / email intake) describing the model-behavior regression on `claude-opus-4-7` at low/medium effort, with the same fo-log artifacts attached or linked.
- Pass: BOTH intakes are completed independently. Each intake's URL (or, for non-public intakes, the alternate intake reference + artifact submission timestamp) is recorded in this entity's body — either in a `## Cross-references` update at the top of the file or under a new `### Upstream issues` subsection in the cycle-3 / cycle-5 sections. AC-F4 does NOT pass with only one intake completed; both surfaces require independent reports.
- Verified by: the entity body contains both intake references; each URL (where applicable) resolves. If either intake is not public-issue-shaped, the non-public-issue fallback prose (alternate intake, artifact submission timestamp) applies independently per surface.

### Test plan

- **Cost**: ~45-90 min for the shared-core prose edit + `claude-team verify-completion` helper + three unit-test cases (AC-F1, AC-F2). Existing live tests continue to run on existing budget (AC-F3 — no new live runs needed unless the cross-check materially changes FO timing). Path C is ~45-60 min of issue prose + artifact attachment across both intakes (AC-F4).
- **Risk**: medium. The cross-check rule has to be tight enough to catch real fabrications without flagging legitimate skipped actions or pre-session-window actions. The DONE-claim short-list and evidence-class abstraction is the load-bearing piece; getting either wrong creates either false positives (FO bounces honest workers, especially on cross-session continuations) or false negatives (the regression slips through). Mitigations: AC-F2's NEGATIVE case is the structural safety net for false negatives; AC-F2's PRIOR-SESSION case is the structural safety net for the cross-session false-positive class.
...
- **Static**: AC-F1 (block exists with anchor phrases `tool-call evidence` + `unmatched DONE claim` inside `## Completion and Gates`), AC-F2 (three cases — positive, negative, prior-session-evidence).
...
- DONE: Edit Path-selection rationale, Implementation scope, and AC-F1..F4 in place to incorporate the 5 fixes.
  All 5 fixes folded into the existing `## Final recommendation` section. Path-selection rationale Path B paragraph rewritten to name session-window scope, evidence-class abstraction, and the helper. Path A rationale updated to cite the second confirmation (`keepalive-pipeline` also has no `_mods/` — verified by inspection of `tests/fixtures/keepalive-pipeline/`). Headline (lead-paragraph bold) updated to mention the helper and BOTH Path C intakes. Implementation scope item 1 expanded to specify (a)-(e) including session-window scope, evidence-class abstraction, once-only loop bound (inherited from step 3), and routing order (tool-call cross-check before AC cross-check). New Implementation scope item 2 names the DONE-claim short-list with evidence classes (`commit_made`, `push_made`, `merge_made`, `pr_opened`, `archive_made`, `sendmessage_made`, `write_made`) and explicit MCP variants. Implementation scope item 3 reframes the helper as REQUIRED with three rationales (testability, FO-on-opus self-defense, established `claude-team` convention). Implementation scope item 5 splits Path C into two parallel intakes. AC-F1 anchor changed from `route back` to `unmatched DONE claim` (distinctive — `route back` appears at shared-core lines 124, 165, 167 under the unrelated Feedback Rejection Flow). AC-F2 expanded from 2 cases (positive + negative) to 3 cases (positive + negative + prior-session-evidence), with the third case asserting the session-window scope rule. AC-F4 split to require BOTH CLI and model intakes independently; non-public-issue fallback applies per surface. Test plan and risk paragraphs updated to reflect the third F2 case as the structural safety net for the cross-session false-positive class.
- DONE: Append a `### Feedback Cycles` cycle-4 → cycle-5 entry. The existing cycle-3 → cycle-4 entry stays.
  Added a second bullet to the existing `### Feedback Cycles` subsection (under `## Cross-references`, line 89) naming the cycle-4 → cycle-5 reframe, the captain's 5 staff-review fixes, and the operational concerns surfaced in Implementation Scope item 1. The cycle-3 → cycle-4 entry is preserved verbatim above it.
- DONE: Write a `## Stage Report: ideation (cycle 5)` documenting the 5 fixes.
  This section.

### Summary

Cycle-5 is pure AC-list and contract-prose refinement; the cycle-4 path-selection (B + C, Path A rejected) does not change. All 5 staff-review fixes folded into the existing sections in place: (1) DONE-claim short-list expanded with evidence-class abstraction and session-window scope rule; (2) AC-F1 anchor changed to the distinctive `unmatched DONE claim`; (3) AC-F4 split into two intakes (CLI repo + Anthropic API/model support); (4) helper invocation made REQUIRED with three rationales; (5) AC-F2 third case (prior-session-evidence) added to enforce the session-window scope rule. Operational concerns (MCP tool name mapping via evidence classes, routing order between cross-checks, once-only loop bound) are surfaced inline in Implementation Scope item 1 (a)-(e). Cycle-3 sanity-check section unchanged; both cycle-3 evidence (R1/R2/R3 fo-logs) and cycle-2 audit trail preserved verbatim.
