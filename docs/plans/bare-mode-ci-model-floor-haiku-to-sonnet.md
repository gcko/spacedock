...
- FAILED: Run `make test-live-claude-bare TEST=test_gate_guardrail` from the worktree per AC-6.
  Test exited non-zero (`make: *** [test-live-claude-bare] Error 1`) due to **auth-shape failure, not capability-shape failure**. All sampled `fo-log.jsonl` files under `/tmp/2yd-impl-rerun/spacedock-test-*/` show `"model":"claude-sonnet-4-6"` (confirming the Makefile change correctly pinned sonnet — sub-conditions "fo-log.jsonl `modelUsage` contains `claude-sonnet-`" and "no `claude-haiku-` in bare-job-touched paths" are satisfied), but every run's second log line is `{"type":"system","subtype":"api_retry","error_status":401,"error":"authentication_failed"}`. The FO's `fo-texts.txt` content across all sampled dirs is the single line `Failed to authenticate. API Error: 401 Invalid authentication credentials`. The captain's dispatch explicitly flagged this risk: "If the token has been reverted between ideation and now, your live test will 429 — surface immediately if so." (Same auth-class failure, 401 instead of 429.) `~/.claude/benchmark-token` (md5 `e9ad99d7…`) differs from `~/.claude/benchmark-token.stale-march31` (md5 `197bc174…`), so the current file is NOT the pre-swap stale token — the post-swap token used in the cycle-1 sonnet spike has since rotated/expired on the captain's macOS OAuth keychain. **The Makefile change is verified working; the AC-6 gate is auth-blocked, not capability-blocked.** Per dispatch instruction "If the run fails … do NOT signal complete — surface the failure and stop," I am NOT signaling complete.
